...
verbose(_verbose),
log_ants_paths(_log_ants_paths) {
    ...
    for ( int32_t a=0; a<numberOfAnts; a++ ) {
        ants.push_back( ANT ( a, max_recurrent_depth-1, verbose ) );
    }

    for (int i=0; i<max_recurrent_depth; i++) {
        levels_pheromones.push_back( INITIAL_PHEROMONE * 2 * ( max_recurrent_depth - i ) ) ;
    }
    if ( verbose ) cout << "NUMBER OF LEVELS: " << levels_pheromones.size() <<  endl ;
...
RNN_Genome* COLONY::generateRNN() {
    ...
    for ( int a=0; a<ants.size(); a++ ) {
        ...
        if ( verbose ) cout << "\t Input Node: " << ants[a].path.input->x << endl ;
        while ( iterate ) {
    ...
    if ( verbose ) cout << "* Centroids *\n" ;
    map<int, map<int, POINT*> > levels_ceteroids ;
    for (auto level : levels ) {
        if ( verbose ) cout << "\tLEVEL: " << level.first << endl ;
        ...
        if ( verbose ) {
          for (auto c: ds.centroids) {
              ...
              for (auto i: c.second->node_type_pheromones) {
                  cout <<  "\t\t\t\tNode Type Pheromones: " << i << endl;
              }
          }
        }
        levels_ceteroids[level.first] = ds.centroids ;

        if ( verbose ) cout << "____________________\n\n" ;
    }
...
RNN_Node*  COLONY::buildNode(vector<RNN_Node_Interface*> &rnn_nodes, POINT* point, vector <double> &parameters ) {
	...
	rnn_nodes.back()->parameter_name = point->parameter_name ;

    if ( verbose ) cout << "Number of Entered nodes: " << rnn_nodes.size() << " Node Type: " << point->type << endl ;
    vector<double> ps ;
    ...
    for ( int i=0; i<ps.size(); i++) {
        ...
        parameters.push_back(rng(generator)) ;
        if ( abs(parameters.back() ) > 1 ) {
            std::cerr << "ERROR (COLONY.cxx): Node Weight: " << parameters.back() << " ... THIS SHOULD NOT HAPPEN!" << endl ;
            exit(1) ;
        }
    }
...
RNN_Genome* COLONY::buildGenome( vector<POINT*> picked_input_points ) {
    ...
    for ( auto p1: rnns_blueprints[generated_genomes].segments ) {
      ...
      for ( auto p2: p1.second ) {
        ...
        if ( p1.first->level == p2.second->level || p1.first->type==-1 ) {
            if ( verbose ) cout << "Creating Edge between Node: " << in_node->innovation_number << " and Node: " << out_node->innovation_number << endl ;
    ...
    if ( verbose ) cout << "Number of Input Nodes: " << count << endl;


    for (auto w: edges_parameters ) {
        parameters.push_back( w );
        if ( abs(w) > 1 ) {
            std::cerr << "ERROR (COLONY): Edge Weight: " << w << " ... THIS SHOULD NOT HAPPEN!" << endl ;
            exit(1) ;
        }
    }
    for (auto w: rec_edges_parameters ) {
        parameters.push_back( w );
        if ( abs(w) > 1 ) {
            std::cerr << "ERROR (COLONY): RecEdge Weight: " << w << " ... THIS SHOULD NOT HAPPEN!" << endl ;
            exit(1) ;
        }
    }
...
rnn_segment_type COLONY::createSegments( map<int, map<int, POINT*> > centeroids ) {
    ...
    stringstream str ;
    if ( verbose ) cout << "*Creating RNN Segments*\n" ;
    if ( verbose ) cout << "*---------------------*\n" ;
    for ( ANT ant: ants ) {
        if ( verbose ) cout << "\tAnt(" << ant.id << ")" << endl;
...
void COLONY::evaporatePheromone () {
    if ( verbose ) cout << "Start Evaporation\n" ;
    ...
    for (int i=0; i<levels_pheromones.size(); i++) {
        ...
        if ( levels_pheromones[i] < INITIAL_PHEROMONE * 2 * ( max_recurrent_depth - i ) ) {
            levels_pheromones[i] = INITIAL_PHEROMONE * 2 * ( max_recurrent_depth - i ) ;
        }
    }
    ...
    for (map <int, map<int32_t, POINT*> >::iterator level=levels.begin(); level!=levels.end(); ++level) {
        ...
        for (map<int32_t, POINT*>::iterator points=level->second.begin(); points!=level->second.end(); ++points) {
            ...
            if ( level->first == 0) {
                if ( verbose ) cout << "AFTER: "  << points->second->pheromone << "  ID: " << points->second->id << endl;
                if ( verbose ) cout << "X: " << points->second->x << " **Y: " << points->second->y << endl;
                if ( verbose ) cout << "____________________\n" ;
            }
        ...
        for ( int32_t p_id: perishing_points_id ) {
            if ( verbose ) cout << "Evaporation Removing: " << "Point level: " << levels[level->first][p_id]->level << endl ;
            levels[level->first].erase( p_id ) ;
        }
    }
    if ( verbose ) cout << "Finish Evaporation\n" ;
}

void COLONY::depositePheromone( map < POINT*, map<int32_t, POINT*> > &segments, RNN_Genome* genome ) {
    if ( verbose ) cout << "Depositing Pheromone...\n" ;
    ...
    if ( verbose ) cout << "Finished Depositing Pheromone!\n" ;
}
...
void COLONY::writeColonyToFile (int number) {
    ...
    outfile.open(output_directory + "/points_"+to_string(number)+".txt"); // append instead of overwrite
    ...
    stringstream s2;
    pathsfile.open(output_directory + "/paths_"+to_string(number)+".txt"); // append instead of overwrite
    ...
    while (true) {
        ...
        if (tag == WORK_REQUEST_TAG) {
            ...
            if ( generated_counts <= max_genomes ) {
              genome = generateRNN();
              if ( log_ants_paths ) writeColonyToFile(genome->get_generation_id()) ;
            }
            colony_mutex.unlock();
...
bool COLONY::insert_genome(RNN_Genome* genome) {
    ...
    if ( verbose ) cout << "genomes evaluated: " << setw(10) << inserted_genomes << ", inserting: " << parse_fitness(genome->get_fitness()) << endl;

    if (population.size() >= population_size  && new_fitness > population.back()->get_fitness()) {
        if ( verbose ) cout << "ignoring genome, fitness: " << new_fitness << " > worst population" << " fitness: " << population.back()->get_fitness() << endl;
        print_population();
        return false;
    }
    ...
    if (duplicate_genome >= 0) {
        ...
        if ( verbose ) cout << "found duplicate at position: " << duplicate_genome << endl;
        ...
        if (duplicate->get_fitness() > new_fitness) {
            ...
            if ( verbose ) cout << "REPLACING DUPLICATE GENOME, fitness of genome in search: " << parse_fitness(duplicate->get_fitness()) << ", new fitness: " << parse_fitness(genome->get_fitness()) << endl;
    ...
    if (population.size() < population_size || population.back()->get_fitness() > new_fitness) {
        ...
        if ( verbose ) cout << "inserting new genome to population " << endl;
        ...
        was_inserted = false;
        if ( verbose ) cout << "not inserting genome due to poor fitness" << endl;
    }

    print_population();

    if ( verbose ) cout << "printed population!" << endl;
...
int main(int argc, char** argv){
    ...
    bool verbose = false;
    get_argument(arguments, "--verbose", true, verbose);

    bool log_ants_paths = false;
    get_argument(arguments, "--log_ants_paths", true, log_ants_paths);
